...
export default function CartPage() {
  ...
  const buildWhatsAppMessage = () => {
    const lines = selectedItems.map((it: any, idx: number) => {
      ...
      const variantText = getVariantText(it?.product, it?.variantId);
      const color = it?.colorKey ? `, Color: ${it.colorKey}` : "";
      const qty = Number(it?.qty || 1);
      const price = Number(it?.salePrice || 0);

      return `${idx + 1}) ${title} | ${variantText}${color} | Qty: ${qty} | ₹${(price * qty).toFixed(0)}`;
    });

    lines.push(``);
    lines.push(`Total: ₹${Number(subtotal || 0).toFixed(0)}`);
  ...
  const openWhatsAppCheckout = () => {
    if (!selectedItems.length) {
      setError("Please select at least 1 item to order on WhatsApp.");
      return;
    }
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-6 rounded-3xl border bg-white p-5">
            ...
            <button
              ...
              className="mt-6 w-full rounded-2xl bg-green-600 px-4 py-3 text-sm font-semibold text-white hover:bg-green-700"
            >
              Order on WhatsApp
            </button>
